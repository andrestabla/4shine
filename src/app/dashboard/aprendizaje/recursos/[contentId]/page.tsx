            ...
            <div className="p-4 space-y-2">
              {resource.contentType === "scorm" && flatItems.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`truncate text-sm font-bold ${isActive ? "text-[va(--brand-primary)] text-[#e85d24]" : "text-[var(--app-ink)]"}`}>
                          {item.title}
                        </p>
                        {item.durationLabel && (
                          <div className="mt-1 flex items-center gap-1.5 text-[10px] text-slate-500 font-medium">
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"></rect><line x1="16" x2="16" y1="2" y2="6"></line><line x1="8" x2="8" y1="2" y2="6"></line><line x1="3" x2="21" y1="10" y2="10"></line><path d="m9 16 2 2 4-4"></path></svg>
                            {item.durationLabel}
                          </div>
                        )}
                      </div>
                ...
                <div className="px-2 py-4">
                  <p className="text-sm font-medium text-[var(--app-ink)]">{resource.title}</p>
                  <p className="text-xs text-[var(--app-muted)] mt-1">{learningContentTypeLabel(resource.contentType)}</p>
                </div>
              )}
            </div>
          ) : (
 ...
 picture-in-picture"
               ...
               <div className="w-full overflow-hidden rounded-[16px] border border-slate-800 bg-[#0f172a] shadow-2xl md:aspect-video flex flex-col justify-between">
                 ...
                 <div className="p-8 pb-10 bg-gradient-to-t from-black/60 to-transparent">
                    <h2 className="text-2xl font-bold text-white mb-2">{currentItem?.title ?? resource.title}</h2>
                    <p className="text-slate-300 text-sm">{resource.description ?? "En este módulo aprenderemos los principales conceptos."}</p>
                 </div>
               </div>
            )}
          </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20 flex h-20 items-center justify-between border-t border-slate-800 bg-[#1e293b] px-6 text-white md:px-12">
           ...
           <div className="text-sm font-semibold text-slate-400">
             Módulo {activeResourceIndex + 1} de {totalItems}
           </div>
