...
import { getUserDetail, hardDeleteUser, updateUser } from '@/features/usuarios/service';
  ...
  try {
    const data = await withClient((client) =>
      withRoleContext(client, identity.userId, identity.role, async () => {
        const result = await getUserDetail(client, userId);
        await logModuleAudit(client, request, identity, {
          moduleCode: 'usuarios',
          action: 'query_user_detail',
    ...
    return errorResponse(error, 'Failed to load user detail');
  }
}
