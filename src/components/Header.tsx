...
import { Bell, Search, Menu, LogOut, X, Check, Info, AlertCircle, MessageSquare } from 'lucide-react';
...
export default function Header({ onMenuClick }: { onMenuClick?: () => void }) {
    ...
    // Capitalize view name for title
    ...
    return (
        <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-10 px-4 md:px-8 py-4 flex justify-between items-center transition-all duration-300">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-xl md:text-2xl font-bold text-slate-800 capitalize truncate max-w-[200px] md:max-w-none">
                    {currentView === 'dashboard' ? `Hola, ${currentUser?.name.split(' ')[0]}` : pageTitle}
                </h1>
            </div>
                    ...
                    {showNotifications && (
                        <div className="absolute right-0 mt-4 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden animate-fade-in origin-top-right">
                            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                ...
                                {unreadCount > 0 && (
                                    <button onClick={markAllRead} className="text-xs text-amber-600 font-bold hover:underline">Marcar leídas</button>
                                )}
                            </div>
                            ...
                            <div className="p-3 text-center border-t border-slate-100 bg-slate-50/50">
                                <button className="text-xs font-bold text-slate-500 hover:text-slate-800">Ver todas</button>
                            </div>
                        </div>
                    )}
                </div>
