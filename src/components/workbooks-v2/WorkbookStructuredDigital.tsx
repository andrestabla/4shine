...
function downloadWorkbookHtml(config: StructuredWorkbookConfig, values: Record<string, string>) {
    const html = `
        <!doctype html>
        <html lang="es">
            <head>
            ...
            <body style="font-family:Arial,Helvetica,sans-serif;background:#f4f7fb;color:#0f172a;margin:0;padding:32px;">
                <main style="max-width:960px;margin:0 auto;background:#ffffff;border:1px solid #dbe4ee;border-radius:24px;padding:32px;">
                    ...
                    <p style="font-size:13px;color:#64748b;margin:0;">${escapeHtml(config.sourceLabel)}</p>
            ...
            <div className="mx-auto grid max-w-[1280px] gap-6 px-3 py-6 sm:px-5 md:px-8 lg:grid-cols-[280px_minmax(0,1fr)]">
                <aside className={WORKBOOK_V2_EDITORIAL.classes.sidebar}>
                    ...
                    <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Fuente</div>
                        <p className="mt-2 text-sm leading-relaxed text-slate-600">{config.sourceLabel}</p>
                    </div>
                </aside>
